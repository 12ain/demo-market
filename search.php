<?php
	...
	include 'connect.php';
	$sql = "SELECT * FROM business_goods WHERE Gname LIKE '%{$keywords}%'";

	
	$mysql_resultt = $db -> query($sql);

	
	$rows = [];
	while($row = $mysql_resultt->fetch_array(MYSQLI_ASSOC)){
	    $rows[] = $row;
	}
